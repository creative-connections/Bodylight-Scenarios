...
V následující simulaci a ze znalosti z předchozích simulací rozhodněte, který patologický stav může vést k malabsorpci železa a k jakému klinickému obrazu to může vést.

<bdl-quiz question="1. Pusťte simulaci a rozhodněte čím způsobíte malabsorpci železa?" answers="knockout genu pro ferroportin|knockout genu pro hepcidin" correctoptions="true|false" explanations="ano, při knockoutování tohoto genu se absorpce sníží až zastaví|ne, knockoutováním tohoto genu se jen zruší regulace absorpce železa z enterocytu" buttontitle="zkontrolovat odpověď"></bdl-quiz>
<bdl-quiz question="2. Pokračujte v simulaci. Nastavte správný parametr z předchozího úkolu. Simulujte aspoň 48 h (simulační čas). Co pozorujete u ostatních indikátorů" answers="koncentrace hepcidinu se snižuje|koncentrace hepcidinu se zvyšuje" correctoptions="true|false" explanations="ano|ne" buttontitle="zkontrolovat odpověď"></bdl-quiz>
